  ...
  <div class="py-4 sm:px-4 lg:px-8 max-w-full">
    <div class="w-full bg-white dark:bg-gray-800 rounded-md border shadow-md text-black p-4 sm:p-6">
      ...
      <form class="mt-6 max-w-full" method="GET" action="">
        <label for="default-search" class="mb-2 text-sm font-medium text-gray-900 sr-only">Search Contacts</label>
        <div class="relative">
        ...
        <table class="min-w-full divide-y divide-gray-300 text-sm sm:text-base">
          <thead class="bg-orange-500 text-white uppercase font-medium  text-center">
            <tr>
              ...
              <th class="px-2 py-3 lg:px-8 lg:py-3">Name</th>
              <th class="px-2 py-3 lg:px-8 lg:py-3">Email</th>
              <th class="px-2 py-3 lg:px-8 lg:py-3">Date</th>
          ...
          <tbody class="bg-gray-100 text-gray-900  text-center">
            <?php if($contacts) :?>
              <?php foreach($contacts as $contact) :?>
                <tr class="border-b">
                  <td class="px-2 py-3 lg:px-8 lg:py-2"><?= $contact->id ?></td>
                  <td class="px-2 py-3 lg:px-8 lg:py-2"><?= htmlspecialchars($contact->name) ?></td>
                  <td class="px-2 py-3 lg:px-8 lg:py-2"><?= htmlspecialchars($contact->email) ?></td>
                  <td class="px-2 py-3 lg:px-8 lg:py-2"><?= htmlspecialchars($contact->created_at) ?></td>
                    <?php 
                        $maxLength = 30;
                  ...
                  <td class="px-4 py-2">
                    <div class="flex flex-wrap gap-2 ">
                       <a href="#" class="text-green-600 whitespace-nowrap px-2" data-id="<?= $contact->id ?>" id="contact-show">
                        <i class="fa-solid fa-pen-to-square"></i> Show
                      </a>
                      <a href="#" class="text-blue-600 whitespace-nowrap px-2" data-id="<?= $contact->id ?>" id="contact-edit">
                        <i class="fa-solid fa-pen-to-square"></i> Edit
                      </a>
                      <form id="contact-delete" data-id="<?= $contact->id ?>" class="inline">
                        <?= csrf_token() ?>
                        <button type="submit" class="contact-delete text-red-600 whitespace-nowrap">
      ...
      <div class="block lg:hidden mt-6 space-y-4">
        <?php if($contacts) :?>
            ...
            <div class="bg-gray-100 dark:bg-gray-300 p-4 rounded-md shadow">
                ...
                <div class="font-semibold text-lg text-orange-600">Contact Us Message #<?= $contact->id ?></div>
                <div class="flex gap-3">
                    <a href="#" class="text-green-600 whitespace-nowrap px-2" data-id="<?= $contact->id ?>" id="contact-show">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" class="text-blue-600" data-id="<?= $contact->id ?>" id="contact-edit">
                    <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <form id="contact-delete" data-id="<?= $contact->id ?>">
                    <?= csrf_token() ?>
                    <button type="submit" class="contact-delete text-red-600">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
                    </form>
                </div>
                </div>
                <div><strong>User: </strong> <?= htmlspecialchars($contact->name) ?></div>
                <div><strong>Car: </strong> <?= htmlspecialchars($contact->email) ?></div>      
                <div><strong>Created at: </strong> <?= htmlspecialchars($contact->created_at) ?></div>             
                <div class="mt-2 whitespace-pre-wrap"><?= htmlspecialchars($contact->message) ?></div>
            </div>
            <?php endforeach; ?>
        <?php endif; ?>
        </div>
